...
// Нет STW (stop-the-world) — перенос растянут по времени</code></pre>
          ...
          <details class="deep" style="margin:10px 0">
            ...
            <div class="deep-body">
              <p class="tight">Старая реализация (bmap) хранила ключи и значения отдельными массивами внутри бакета: сначала все ключи, потом все значения. Чтобы прочитать пару k₀→v₀, процессор прыгал в одно место памяти за ключом, потом в совсем другое — за значением. Это означало два обращения к кэшу — дорого.</p>
              <p class="tight">В <b>Swiss Tables</b> (Go 1.24+) каждая пара хранится <b>рядом</b> — k₀ и v₀ лежат в одном слоте, бок о бок. Ключ нашли — значение уже тут, в одной кэш-линии. Кроме того, каждая группа из 8 слотов несёт компактное <b>control word</b> — 8 байт с информацией о занятости. Поиск по всем 8 слотам сразу выполняется одной операцией вместо цикла. Итог: быстрее нахождение, меньше cache miss-ов, меньше памяти на метаданные.</p>
              <p class="tight" style="color:var(--text-dim);font-size:12px">Структура: <code class="inline">Map</code> → <code class="inline">Directory</code> → <code class="inline">Table[]</code> → <code class="inline">Group[]</code> → 8 слотов + control word. Extendible hashing: старшие <code class="inline">globalDepth</code> бит хеша выбирают таблицу из directory.</p>
